...
device on the local network, and everyone else just opens
[kibitz.chat](https://kibitz.chat) in a browser on that same Wi-Fi — and you're
in a call. The video flows phone-to-phone across the LAN; this relay only helps
the browsers find each other.

> Status: **beta.** The call engine is battle-tested (a real-world, real-device
> voice/video mesh); the offline-relay flow is newer. Works today; rough edges
> possible.
...
- **The relay can't see or hear your audio/video.** Voice and video are a
  peer-to-peer WebRTC mesh, encrypted end-to-end (DTLS-SRTP) and flowing
  browser-to-browser — the relay only helps the browsers find each other; it
  never carries, decrypts, records, or joins your media.
- **It does route the coordination data — and that's your own data.** Be precise:
  unlike an internet TURN relay (which blindly forwards encrypted packets it
  can't read), this LAN relay is the **coordination hub** — the roster, chat, and
  any app messages are routed *through* it between browsers, so it can see that
  text. That's fine here: it's a box *you* run on *your own* Wi-Fi, so "the relay
  can see the data" just means "you can see your own data." Nothing leaves your
  network, and there's no operator but you.
- **No accounts, no servers, no internet.** Nothing phones home. The relay binds
...
a server is for. This relay sidesteps that by having a **permanent identity**: a
...
the relay's side of the handshake *locally* from that blob — so there's **zero
...
[kibitz.chat setup page](https://kibitz.chat/#relay)):

| File | For |
|------|-----|
| `kibitz-relay-windows.exe` | Windows 10/11 |
| `kibitz-relay-mac-apple-silicon` | M1/M2/M3 Mac |
| `kibitz-relay-mac-intel` | older Intel Mac |
| `kibitz-relay-linux` | 64-bit x86 Linux |
| `kibitz-relay-raspberry-pi` | 64-bit Raspberry Pi OS (Pi 3/4/5) |
...
- **macOS / Linux / Pi:** make it executable, then run:
  ```sh
  chmod +x kibitz-relay-*
  ./kibitz-relay-*          # macOS: first run, right-click → Open (Gatekeeper)
  ```
...
> **No screen on the box?** (a headless Raspberry Pi) The relay also serves the
> same QR over HTTP — open `http://<the-relay's-IP>:8080/` from any phone on the
> Wi-Fi to see, scan, screenshot, or print it.

> **Truly no internet at the venue?** (the relay's own hotspot, a dead-zone
...
```sh
git clone <this repo> && cd kibitz-relay
go build -o kibitz-relay .     # your platform
./kibitz-relay                 # run it
```
...
A Pi makes a perfect set-and-forget appliance: boots → relay runs → the join
code never changes. See [`pi/README.md`](pi/README.md) for the 5-minute systemd
setup (it ships in every release too).

## Android

The relay also runs as an Android app, so a spare phone becomes the box — it can
keep relaying across reboots. That app ships through the kibitz.chat setup page
(Play Store listing pending); the same `mobile/` package here is what it binds
...
MIT licensed. Built from a battle-tested in-call WebRTC engine.
