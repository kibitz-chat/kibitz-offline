...
[ -n "$BASE" ] && LDFLAGS="$LDFLAGS -X kibitz-offline/relaycore.DefaultLinkBase=$BASE"
...
build windows amd64 kibitz-offline-windows.exe
build darwin  arm64 kibitz-offline-mac-apple-silicon
build darwin  amd64 kibitz-offline-mac-intel
build linux   amd64 kibitz-offline-linux
build linux   arm64 kibitz-offline-raspberry-pi

# The Pi appliance kit travels with the release so the systemd guide is one click away.
cp pi/kibitz-offline.service pi/README.md dist/
